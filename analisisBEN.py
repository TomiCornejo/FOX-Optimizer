...
for instancia in instancias:
    ...
    for d in blob:
        ...
        if mh == "FOX" and incluye_fox == False:
            archivoResumenFitness.write(",best,avg. fitness, std fitness")
            archivoResumenTimes.write(", min time (s), avg. time (s), std time (s)")
            archivoResumenPercentage.write(", avg. XPL%, avg. XPT%")
            incluye_fox = True
        ...
        if mh == 'FOX':
            fitnessFOX.append(np.min(fitness))
            timeFOX.append(np.round(np.sum(time),3))
            xplFOX.append(np.round(np.mean(xpl), decimals=2))
            xptFOX.append(np.round(np.mean(xpt), decimals=2))
            archivoFitness.write(f'FOX,{str(np.min(fitness))}\n')
    ...
    if incluye_fox:
        archivoResumenFitness.write(f''',{np.min(fitnessFOX)},{np.round(np.average(fitnessFOX),3)},{np.round(np.std(fitnessFOX),3)}''')
        archivoResumenTimes.write(f''',{np.min(timeFOX)},{np.round(np.average(timeFOX),3)},{np.round(np.std(timeFOX),3)}''')
        archivoResumenPercentage.write(f''',{np.round(np.average(xplFOX),3)},{np.round(np.average(xplFOX),3)}''')
    ...
    if incluye_fox:
        axPER.plot(iteraciones, bestFitnessFOX, color="orange", label="FOX")
